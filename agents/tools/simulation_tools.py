...
def create_simplified_wind_conditions(wind_conditions, top_n=5):
    ...
    logger.info(f"Creating simplified wind conditions with top {top_n} sectors")
    ...
    try:
        ...
        logger.info(f"Simplified wind conditions created successfully with {len(simplified_conditions['p_wd'])} sectors")
...
def create_wind_turbine(turbine_model: str) -> WindTurbine:
    ...
    try: 
        if turbine_model is not None:
            ...
            logger.info(f"Turbine specs retrieved: {turbine_specs is not None}")
            if turbine_specs:
...
def run_simplified_flow_map_simulation(simplified_conditions, turbines_geojson, turbine_model=None):
    ...
    try:
        ...
        logger.info(f"Processing {len(turbines_geojson['features'])} turbines for flow map")
        ...
        logger.info(f"Converted {len(turbine_positions)} turbine positions to local coordinates")
...
def run_wake_simulation(wind_conditions: Dict, project_id: str, calculate_flow_map: bool = True, turbine_model: Optional[str] = None) -> Dict:
    ...
    try:
        ...
        center_lon = np.mean([f['geometry']['coordinates'][0] for f in turbines_geojson['features']])
        center_lat = np.mean([f['geometry']['coordinates'][1] for f in turbines_geojson['features']])
        logger.info(f"Processing {len(turbines_geojson['features'])} turbines at center coordinates: lon={center_lon:.4f}, lat={center_lat:.4f}")
        ...
        logger.info(f"Wind conditions: {len(p_wd)} sectors, mean a={np.mean(a):.2f}, mean k={np.mean(k):.2f}, ti={ti:.2f}")
    ...
    try:
        ...
        x_positions = positions[:, 0]
        y_positions = positions[:, 1]
        logger.info(f"Position arrays created: x_range=[{min(x_positions):.1f}, {max(x_positions):.1f}], y_range=[{min(y_positions):.1f}, {max(y_positions):.1f}]")
    ...
    try:
        ...
        logger.info(f"Cache updated, current keys: {list(SIMULATION_CACHE.keys())}")
...
def generate_charts_and_csv(project_id: str, simulation_id: str) -> Dict:
    ...
    try:
        ...
        df.to_csv(csv_buffer, index=True)        # Maintain index if you want coordinates in CSV
        ...
        with tempfile.NamedTemporaryFile(suffix='.json', delete=False, mode='w', encoding='utf-8') as temp_file:
            json.dump(serializable_summary, temp_file, indent=4)  # Serialize dict to JSON and write to file
