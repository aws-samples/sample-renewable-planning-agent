...
def query_overpass(lat, lon, radius_km=5, residence_setback_m=300, road_setback_m=110, lines_setback_m=110, water_setback_m=30.48, default_setback_m=100):
    ...
    logger.info(f"Starting Overpass query for lat={lat}, lon={lon}, radius={radius_km}km")
    ...
    logger.info(f"OSM data contains {len(osm_data.get('elements', []))} elements")
    
    geojson_data = osm_to_geojson(osm_data)
    logger.info(f"Converted to GeoJSON with {len(geojson_data.get('features', []))} features")
    
    # Filter features within radius
    geojson_data = filter_by_radius(geojson_data, lat, lon, radius_km)
    logger.info(f"After radius filter: {len(geojson_data.get('features', []))} features")
    
    # Apply setback with different distances based on feature type
    geojson_data = apply_setback(geojson_data, residence_setback_m, road_setback_m, lines_setback_m, water_setback_m, default_setback_m)
    logger.info(f"After setback: {len(geojson_data.get('features', []))} features")
    
    # Simplify by merging geometries
    geojson_data = simplify_geojson_union(geojson_data)
    logger.info(f"After simplification: {len(geojson_data.get('features', []))} features")
...
def simplify_geojson_union(geojson_data):
    ...
    try:
        ...
        building_mask = gdf['building'].notna() if 'building' in gdf.columns else pd.Series([False] * len(gdf), index=gdf.index)
        highway_mask = gdf['highway'].notna() if 'highway' in gdf.columns else pd.Series([False] * len(gdf), index=gdf.index)
        natural_water_mask = (gdf['natural'] == 'water') if 'natural' in gdf.columns else pd.Series([False] * len(gdf), index=gdf.index)
        waterway_mask = gdf['waterway'].notna() if 'waterway' in gdf.columns else pd.Series([False] * len(gdf), index=gdf.index)
        ...
        for group_name, group_gdf in groups.items():
            if not group_gdf.empty:
                ...
                merged_geom = merged_geom.simplify(tolerance=0.0003, preserve_topology=True)
...
def save_analysis_results(geojson_data, latitude, longitude, project_id):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Requesting satellite imagery from USGS: {response.status_code}")
            if response.status_code == 200:
                ...
                if not gdf_plot.empty:
                    ...
                    if 'feature_type' in gdf_plot.columns:
                        ...
                        gdf_plot.plot(ax=ax, color='purple', alpha=0.6, edgecolor='darkviolet', linewidth=1)
...
def get_unbuildable_areas(latitude: float, longitude: float, project_id: str, 
    ...
    try:
        ...
        logger.info(f"Parameters: radius_km={radius_km}, default_setback_m={default_setback_m}, project_id={project_id}")
        logger.info(f"Turbine specs: tip_height_m={tip_height_m}, rotor_radius_m={rotor_radius_m}")
        logger.info(f"Setbacks in prompt: residence={residence_setback_m}, roads={road_setback_m}, lines={lines_setback_m}, water={water_setback_m}")
        ...
        terrain_data = query_overpass(latitude, longitude, radius_km, residence_setback_m, road_setback_m, lines_setback_m, water_setback_m, default_setback_m )
        logger.info(f"Overpass query completed. Found {len(terrain_data.get('features', []))} features")
