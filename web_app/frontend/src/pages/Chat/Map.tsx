...
const Map = ({ geojson, geojsonLayers, selectedCenterpoint, onLocationClick }: MapProps) => {
    ...
    useEffect(() => {
        ...
        vectorLayer.current = new VectorLayer({
            ...
            style: (feature) => {
                ...
                const featureType = properties?.feature_type || properties?.type || 'other';
                ...
                const featureStyles: Record<string, { fillColor: string; strokeColor: string }> = {
                    water: { fillColor: 'rgba(59, 130, 246, 0.6)', strokeColor: '#1e40af' },
                    roads: { fillColor: 'rgba(245, 158, 11, 0.6)', strokeColor: '#d97706' },
                    buildings: { fillColor: 'rgba(239, 68, 68, 0.6)', strokeColor: '#dc2626' },
                    boundary: { fillColor: 'rgba(0, 102, 204, 0.3)', strokeColor: '#0066cc' },
                    exclusion: { fillColor: 'rgba(239, 68, 68, 0.3)', strokeColor: '#dc2626' },
                    suitable: { fillColor: 'rgba(34, 197, 94, 0.3)', strokeColor: '#16a34a' },
                    other: { fillColor: 'rgba(139, 92, 246, 0.6)', strokeColor: '#7c3aed' }
                };
        ...
        map.current.on('pointermove', (event) => {
            ...
            if (feature && feature instanceof Feature && vectorLayer.current?.getSource()?.hasFeature(feature)) {
                const properties = feature.getProperties();
                const featureType = properties?.feature_type || properties?.type || 'Unknown';
                const geometry = feature.getGeometry();
                const geometryType = geometry?.getType();
                ...
                if (featureType === 'turbine') {
                    content += `<strong>ID:</strong> ${properties?.id || properties?.turbine_id || 'N/A'}<br/>`;
                    content += `<strong>Capacity:</strong> ${properties?.capacity || 'N/A'} MW`;
    ...
    useEffect(() => {
        ...
        geojsonLayers.forEach(layer => {
            ...
            else if (filename.includes('layout') || filename.includes('turbine')) agentType = 'layout';
            ...
            features.forEach((feature) => {
                ...
                if (agentType === 'layout' && feature.getGeometry()?.getType() === 'Point') {
                    feature.set('feature_type', 'turbine');
                    feature.set('id', `turbine_${Math.random().toString(36).substr(2, 9)}`);
    ...
    useEffect(() => {
        ...
        const marker = new Feature({
            geometry: new Point(fromLonLat([selectedCenterpoint.lon, selectedCenterpoint.lat]))
        });
        markerLayer.current.getSource()?.clear();
        markerLayer.current.getSource()?.addFeature(marker);
    }, [selectedCenterpoint]);
    ...
    useEffect(() => {
        ...
        if (isSatellite) {
            baseLayer.current.setSource(new XYZ({
