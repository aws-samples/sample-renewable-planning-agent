...
def get_nrel_api_key():
    """Get NREL API key from AWS SSM or environment variable"""
    ...
    try:
        ...
        response = ssm.get_parameter(
            Name='/wind-farm-assistant/nrel_config',
            WithDecryption=False
        )
        nrel_config = response['Parameter']['Value']
        keys = nrel_config.split(",")
        return random.choice(keys)
    except Exception as e:
        # Use `DEMO_KEY` if no key is found in SSM
        logger.warning(f"Unable to retrieve NREL API key from SSM: {e}")
...
def fetch_wind_data(latitude, longitude, year=2023):
    ...
    nrel_api_url = (f'http://developer.nrel.gov/api/wind-toolkit/v2/wind/wtk-bchrrr-v1-0-0-download.csv?'
        ...
        f'years={year}&'
        f'email=nrg-workshops@amazon.com')
    ...
    response = requests.post(nrel_api_url, headers=headers, timeout=60)
